try {
    ...
    function addCountries(selectObject, defaultValueByCountryCode) {
        ...
        if (defaultValueByCountryCode == 'US' || defaultValueByCountryCode == 'CA' || defaultValueByCountryCode == 'AU' || defaultValueByCountryCode == 'GB') {
            selectObject.append('<option value="' + defaultValueByCountryCode + '" class="different option" selected>' + countryList[defaultValueByCountryCode] + '</option>');
        } else {
            for (var code in countryList) {
                selectObject.append('<option value="' + code + '" class="different option"'+(defaultValueByCountryCode == code ? ' selected' : '')+'>' + countryList[code] + '</option>');
            }
        }
    ...
    function update_country_state() {
        ...
        if (countryStateDict[$('#id_country').val()]) {
            ...
            $('#id_state').append('<option value="" disabled selected>'+($('#id_country').val() === 'GB' ? 'Select County' : 'Select State')+'</option>');
        ...
        if ($('#id_country_mobile').length > 0) {
            if (countryStateDict[$('#id_country_mobile').val()]) {
                ...
                $('#id_state_mobile').append('<option value="" disabled selected>'+($('#id_country_mobile').val() === 'GB' ? 'Select County' : 'Select State')+'</option>');
        ...
        if ($('#id_country_sp').length > 0) {
            if (countryStateDict[$('#id_country_sp').val()]) {
                ...
                $('#id_state_sp').append('<option value="" disabled selected>'+($('#id_country_sp').val() === 'GB' ? 'Select County' : 'Select State')+'</option>');
    ...
    if ((iso_code == 'US' || iso_code == 'CA' || iso_code == 'AU') && typeof geodata != 'undefined') {
        ...
        try {
            ...
            if (geodata['subdivisions_iso_code'] != '') {
                ...
                // set city
                if (iso_code == 'US' && geodata['city'] != '' && $('input[type="text"][name="city"]').val() == '') {
