...
function validate_form(){
	...
	$("#firstName").addClass('valid').removeClass('error');
	...
	if ($("#firstName").val().replace(/\s/g,"") =='' || $('#firstName').val()=='') {
...
/*		$("#firstName_e").addClass('error').removeClass('valid');*/
	...
	else if(!filter.test($('#firstName').val())){
		$("#firstName").addClass('error').removeClass('valid');
/*		$("#firstName_e").addClass('error').removeClass('valid');*/
		var label = $("#firstName").closest(":has('label')").find('label');
		alertText.push("First Name");
...
function validate_checkout_form2(){
    skip = 1;
	var alertText = new Array();
	var errors = new Array();
	var filter = /[a-z A-Z]$/;
...
function validate_formthanks(){
	...
	var flag = 0;
	var fcs = '';	
	$("#order_process").addClass('valid').removeClass('error');
	$("#reason").addClass('valid').removeClass('error');
	$("#achieve").addClass('valid').removeClass('error');
	$("#find_us").addClass('valid').removeClass('error');
	
	

		if ($("#order_process").val().replace(/\s/g,"") =='' || $('#order_process').val()=='') {
		$("#order_process").addClass('error').removeClass('valid');
		alertText.push("What did you think of the order process");
		flag = 1;
		if(fcs =='')
				fcs = 'order_process';
	}
	else if(!filter.test($('#order_process').val())){
		$("#order_process").addClass('error').removeClass('valid');
		var label = $("#order_process").closest(":has('label')").find('label');
		alertText.push("What did you think of the order process");
		flag = 1;
		if(fcs =='')
				fcs = 'order_process';
	...
	if ($("#reason").val().replace(/\s/g,"") =='' || $('#reason').val()=='') {
		$("#reason").addClass('error').removeClass('valid');
		alertText.push("What is the most important reason you ordered");
		flag = 1;
		if(fcs =='')
				fcs = 'reason';
	}

	if ($("#achieve").val().replace(/\s/g,"") =='' || $('#achieve').val()=='') {
		$("#achieve").addClass('error').removeClass('valid');
		alertText.push("What do you wish to achieve with this product");
		flag = 1;
		if(fcs =='')
				fcs = 'achieve';
	}
	if ($("#find_us").val().replace(/\s/g,"") =='' || $('#find_us').val()=='') {
		$("#find_us").addClass('error').removeClass('valid');
		alertText.push("How did you find us");
		flag = 1;
		if(fcs =='')
				fcs = 'find_us';
	}
...
/*		$("#email_e").addClass('error').removeClass('valid');*/
	...
	if (flag == 0 ) {
		        ...
		        $('#loader_area').show();
				//$('#pop_overlay').fadeIn(300);
		        var page_ajax = 'submit_order_limelight.php';
		        //alert($('#form1').serialize());
		       $.ajax({type:'POST', url: '../includes/'+page_ajax, data:$('#order_form').serialize(), success: function(response) {
					  //alert(response);
